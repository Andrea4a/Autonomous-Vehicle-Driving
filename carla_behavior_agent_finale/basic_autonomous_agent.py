...
def get_entry_point():
    """
    Entry point function of the Autonomous Agent.
    """
...
class MyTeamAgent(AutonomousAgent):
    ...
    def run_step(self, input_data, timestamp):
        ...
        """
        # if self._agent is not defined yet
        if not self._agent:
            # searching for the ego actor among all of the actors
            hero_actor = None
            ...
            if self.__show:
                ...
                # tmp = cv2.cvtColor(input_data["Center"][1], cv2.COLOR_BGR2RGB)
                # img = im.fromarray(tmp)
                # img.save(f"team_code/images/{self._index:05d}.png")
                # self._index += 1
    ...
    def destroy(self):
        """
        Destroy function of the Autonomous Agent.
        """
        print("DESTROY")
